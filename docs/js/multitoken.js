...
const bancorConvertersContracts = {};    
...
window.addEventListener('load', async function() {
    ...
    $('#buy-for-eth').bind('click', async function () {
        ...
        {
            const amount = value;
            const minReturn = web3js.utils.toBN(await bancorConvertersContracts.BNT.methods.getReturn(bancorTokens.ETH, bancorTokens.BNT, amount).call()).muln(98).divn(100);
            bntValue = minReturn;
            console.log('bntValue = ' + bntValue.toString());
            console.log('convert', bancorTokens.ETH, bancorTokens.BNT, amount.toString(), minReturn.toString());
            const data = bancorConvertersContracts.BNT.methods.convert(bancorTokens.ETH, bancorTokens.BNT, amount, minReturn).encodeABI().substr(2);
            targets.push(bancorConverters.BNT);
            datas += data;
        ...
        for (let i = 0; i < allTokens.length; i++) {
            ...
            const minReturn = web3js.utils.toBN(await bancorConvertersContracts[tokenName].methods.getReturn(bancorTokens.BNT, bancorTokens[tokenName], amount).call()).muln(98).divn(100);
            console.log('convert', bancorTokens.BNT, bancorTokens[tokenName], amount.toString(), minReturn.toString());
            const data = bancorConvertersContracts[tokenName].methods.convert(bancorTokens.BNT, bancorTokens[tokenName], amount, minReturn).encodeABI().substr(2);

            targets.push(bancorConverters[tokenName]);
            datas += data;
            datasIndexes.push(datas.length/2);
            values.push(amount);
        }
        ...
        if (multitokenTotalSupply == 0) {
            console.log('buyFirstTokensOnApprove');
    ...
    await Promise.all(
        Object.keys(bancorRelays).map(async function(key, index) {
            const bancorRelayContract = new web3js.eth.Contract(ownableABI, bancorRelays[key]);
            bancorConverters[key] = await bancorRelayContract.methods.owner().call();
            bancorConvertersContracts[key] = new web3js.eth.Contract(bancorConverterABI, bancorConverters[key]);
        })
    );
    console.log('bancorTokens = ' + JSON.stringify(bancorTokens));
    console.log('bancorRelays = ' + JSON.stringify(bancorRelays));
    console.log('bancorConverters = ' + JSON.stringify(bancorConverters));
