...
window.addEventListener('load', async function() {
    ...
    $('#sell-for-eth').bind('click', async function () {
        ...
        const sellData = multiSellerContract.methods.sell(
            multitokenContract.options.address,
            multitokenAmount,
            targets,
            '0x' + datas,
            datasIndexes,
            account || multiSellerContract // will fail without account
        ).encodeABI();
